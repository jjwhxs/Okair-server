 ...
 * 封装事件处理相关函数A
 */
define(['logic/model/assets/permanentAssetsInitModel', 'moment'],function(initPage, moment) {
	
	// 清空资产信息表单
	var clearAssetsInfoForm = function() {
		$('#selCategoryEdit1, #selCategoryEdit2, #selCityEdit, #sselEquityTypeEdit').val(null).trigger('change');
	...
	var editButtonClick = function(evt) {
		...
		if(selected!=undefined && selected.length===1) {
			...
			$('#tbxAddressEdit').val(result.address);
			$('#tbxAreaEdit').val(result.area);
			$('#tbxUtilEdit').val(result.util);
			$('#selEquityTypeEdit').selectpicker('val', result.equitytype);
			$('#selCityEdit').selectpicker('val', result.city);
			$('#tbxScaleEdit').val(result.scale);
	...
	var transHistoryClick = function(evt) {
		...
		$('#tblTransHistory').bootstrapTable({
			url:baseUrl + '/logic/trans/permanent/history/' + this.value,
			method:'post',
		...
		$.post(baseUrl + '/logic/assets/permanent/save',
				{
					...
					equittype:$('#selEquityTypeEdit').val(),
					address:$('#tbxAddressEdit').val(),
					area:$('#tbxAreaEdit').val(),
					city:$('#selCityEdit').val(),
					scale:$('#tbxScaleEdit').val(),
					util:$('#tbxUtilEdit').val(),
	...
	var instockButtonClick = function(evt) {
		$.post(baseUrl + '/logic/trans/permanent/instock',{
	...
	var searchButtonClick = function(evt) {
		...
		$('#tblResult').bootstrapTable({
			url:baseUrl + '/logic/assets/permanent/list',
			method:'post',
			contentType: 'application/x-www-form-urlencoded',
			pagination: true,
			singleSelect: true,
